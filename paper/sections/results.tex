...
Table~\ref{tab:stats_results} compares each bank's best recurrent case against \texttt{lstm\_without\_macro} as baseline. RMSE gains are positive in six banks, ranging from 4.91\% to 35.66\%.

\begin{table}[t]
\centering
\caption{Best recurrent case vs baseline (\texttt{lstm\_without\_macro}) significance summary.}
...
The best Transformer case outperforms the best original recurrent case in every bank-level comparison. The average best-per-bank improvements are 38.31\% for RMSE, 42.17\% for MAE, and 41.95\% for MAPE. Table~\ref{tab:transformer_gain_bank} reports the RMSE comparison in detail. These results indicate strong potential for Informer/Autoformer-style modeling, but they are reported as extension evidence because paired statistical tests against the recurrent winners have not yet been regenerated.
...
\begin{tabular}{llclc}
\toprule
Bank & Best recurrent & RMSE & Best Transformer & Improve (\%) \\
\midrule
BBCA & \texttt{bilstm\_data} & 1881.53 & \texttt{autoformer\_macro} & 70.98 \\
BBNI & \texttt{bilstm\_data} & 335.85 & \texttt{informer\_data} & 34.60 \\
BBRI & \texttt{bilstm\_data} & 525.70 & \texttt{informer\_macro} & 27.87 \\
BBTN & \texttt{bilstm\_data\_ga} & 51.44 & \texttt{informer\_data\_random} & 16.40 \\
BDMN & \texttt{lstm\_data} & 120.72 & \texttt{informer\_data} & 36.37 \\
BMRI & \texttt{bilstm\_macro\_ga} & 1388.71 & \texttt{informer\_data\_random} & 36.26 \\
BNGA & \texttt{bilstm\_data} & 244.14 & \texttt{informer\_data} & 45.70 \\
\midrule
Mean & -- & -- & -- & \textbf{38.31} \\
\bottomrule
\end{tabular}
