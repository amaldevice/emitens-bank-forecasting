...
For the supplementary Transformer experiments, the code also supports a budgeted random-search mode. This mode uses the same validation objective and a small candidate budget to generate the \texttt{\_random} Transformer cases and fast baseline estimates; it is separate from the GA workflow used for recurrent-family tuning and should not be read as a completed Transformer GA sweep.
